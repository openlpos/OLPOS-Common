 ...
 * @see
 */
public class OperDisplay extends BaseServiceAdapter implements jpos.services.LineDisplayService14 {
    ...
    public OperDisplay() {
        ...
        int fontSize = Integer.valueOf(propList().getProperty("FontSize", "10")).intValue();
        Font font = new Font(propList().getProperty("Font", "Courier"), Font.PLAIN, fontSize);

        current_row = 0;
        current_col = 0;

        JPanel p = new JPanel(new GridLayout(rows, 1, 0, 0));
        ...
        for (int i = 0; i < rows; i++) {
            ...
            p.add(fields[i]);
            fields[i].setText(propList().getProperty("StartupText", "Please wait..."));
        }
