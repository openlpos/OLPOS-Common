...
/**
 * Traverse the ej and switch to alternate tax rate
 * for each EjTax record.
 *
 * @author  Quentin Olson
 */
public class UseAltTax extends PosEvent {

    /** Simple constructor for dynamic load */
    public UseAltTax() {
    }

    public UseAltTax(PosContext context) {
        setContext(context);
    }

    /**
     * Switch each tax item to the alternate tax rate
     */
    public void engage(int value) {
        ...
        for (int i = 0; i < context().currEj().size(); i++) {
            ...
            if (line.lineType() == EjLine.TAX) {
                ...
                tax.setActiveTaxRate(EjTax.ALTERNATE_TAX);
            }
        }
    }
    ...
    private static String eventname = "UseAltTax";
